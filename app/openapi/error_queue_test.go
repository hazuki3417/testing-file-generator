...
func Test_ErrorQueue_Enqueue(t *testing.T) {
	queue := openapi.NewErrorQueue(5)
	...
	err1 := openapi.Error{Key: "name", Reason: "not empty"}
	queue.Enqueue(err1)
	...
	err2 := openapi.Error{Key: "age", Reason: "not empty"}
	queue.Enqueue(err2)
	...
	data1 := openapi.Error{Key: "key1", Reason: "reason1"}
	queue.Enqueue(data1)
	data2 := openapi.Error{Key: "key2", Reason: "reason2"}
	queue.Enqueue(data2)
	data3 := openapi.Error{Key: "key3", Reason: "reason3"}
	queue.Enqueue(data3)

	assert.EqualValues(t, queue.Size(), 3)
	assert.EqualValues(t, queue.All(), []openapi.Error{data1, data2, data3})
	assert.EqualValues(t, queue.Size(), 0)

	queue.Enqueue(openapi.Error{Key: "key0", Reason: "reason0"})
	queue.Denqueue()
